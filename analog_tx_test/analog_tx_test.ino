...
void loop() {
  ...
  Serial.print("reading: "); Serial.println(sensor_reading);


  if (sensor_reading > 1020 && triggered == false) {
    triggered = true;

    char radiopacket[30] = "reading: ";
    //  strcat(radiopacket, sensor_reading);
    itoa(sensor_reading, radiopacket + 9, 10);
    //  itoa(sensor_reading, radiopacket+13, 10);

    transmit(radiopacket);
  }

  if (sensor_reading < 10 && triggered == true) {
    triggered = false;
